def escape_nested_json(transcription):
    """
    Escapes quotes and special characters in nested JSON within the transcription.
    """
...
def get_quiz_prompt(complexity, transcription):
    # Clean and escape nested JSON in the transcription
    transcription = escape_nested_json(transcription)
...
def get_interview_prompt(complexity, transcription):
    ...
    prompt = (f"Generate a {complexity}-level comprehensive job-interview questions based on the following key concepts described in the transcription: \"{transcription}\". "
              ...
              "Please provide the questions and answers in JSON format, ensuring a clear distinction between the question and the answer. Example of expected format: "
              '{"question": "Does correlation imply causation? Can you explain it with some examples?", '
              '"answer": "No, correlation does not imply causation. Correlation simply measures the degree of association between two variables, while causation indicates that one variable directly affects the other. '
              'For instance, ice cream sales and heat strokes are positively correlated in the summer, but selling more ice cream doesn\'t cause more heat strokes. Both variables are influenced by a third factor, which is the hot weather. '
              'Another example is the correlation between umbrella sales and rainfall. Umbrella sales may increase when it rains, but selling umbrellas doesn\'t cause the rain."}. JSON Interview Output:\n')
